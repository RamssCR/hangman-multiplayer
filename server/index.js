...
io.on('connection', (socket) => {
    ...
    socket.on('check_letter', ({username, lives, letter, allPlayers}) => {
        const { gameWord } = word

        const index = allPlayers.findIndex(player => player.username === username)
        if (index === -1) return io.emit('error', { message: 'Player is not in the In-Game room now' })

        if (gameWord.includes(letter)) {
            io.emit('match', {message: 'MATCH!'})
        } else {
            ...
            allPlayers[index] = modifiedPlayer
            io.emit('players', {
                wordConfig: word,
                allPlayers: allPlayers,
                starting_player: allPlayers[0].username
            })
            io.emit('miss', {message: 'MISS'})
        }
        
        // Passing to the next player's turn
        if (index === allPlayers.length - 1) {
            current_turn = 0
            console.log(allPlayers[current_turn])
            return io.emit('current_turn', {current_turn: allPlayers[current_turn].username})
        }
